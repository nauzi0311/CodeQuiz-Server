...
const { GetCourseText, GetVersionText } = require('./constValue');

const course = GetCourseText();
const config_file = GetCourseDir(course) + 'config.json';
const log_file = './log/' + course + '/log.txt';
...
router.post('/', function(req, res, next) {
  ...
  if(fs.existsSync(user_file)){
    ...
    WriteAddFile("./log/" + course + "/" + GetSchoolNum(device,course) + ".txt",log_data);
...
router.post('/signup', function(req, res, next) {
  ...
  WriteAddFile(log_file,log_data);
  //write user_log
  const new_log_file = "./log/"+ course + "/" + school_num + ".txt";
...
router.post('/version', function(req, res, next) {
  ...
  const response = {"version":version,"url":url,visible:false};
